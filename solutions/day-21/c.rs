use std::{cmp::min, collections::VecDeque, i32, io::stdin, path};
...
fn process(input: &str) -> i32 {
    ...
    // for comb in c1 {
    //     println!("{}", comb.iter().collect::<String>());
    // }

    let mut mn = rec(&c1, &G2, 0);
    // for comb in c1 {
    //     (cx, cy) = (0, 2);
    ...
    println!("len, number = ({}, {})", mn, numeric_part);
    return mn * numeric_part;
}

fn rec(combs: &Vec<Vec<char>>, g2: &Vec<Vec<char>>, depth: i32) -> i32 {
    if depth == 2 {
        let mut res = i32::MAX;
        for comb in combs {
            res = min(res, comb.len() as i32);
        }
        return res;
    }
    let mut res = i32::MAX;
    for comb in combs {
        let (mut cx, mut cy) = (0, 2);
        let mut paths: Vec<Vec<Vec<char>>> = vec![];
        for x in comb {
            paths.push(find_paths(g2, cx, cy, *x));
            update_cur(g2, &mut cx, &mut cy, *x);
        }
        let c2 = cartesian_product(&paths);
        res = min(res, rec(&c2, g2, depth + 1));
    }
    return res;
}
...
fn find_paths(grid: &Vec<Vec<char>>, sx: usize, sy: usize, target_char: char) -> Vec<Vec<char>> {
    ...
    dfs(sx, sy, target_char, grid, &mut vis, &mut _path, &mut all_paths);
...
fn dfs(x: usize, y: usize, target_char: char, grid: &Vec<Vec<char>>, vis: &mut Vec<Vec<bool>>, cur_path: &mut Vec<usize>, all_paths: &mut Vec<Vec<usize>>){
    ...
    for i in 0..4 {
        ...
        {
            cur_path.push(i);
            dfs(nx as usize, ny as usize, target_char, grid, vis, cur_path, all_paths);
            cur_path.pop().unwrap();
        }
    }
    vis[x][y] = false;
